	...
	return (
		<div
			className={cn(
				...
				Array.isArray(documents) && 'md:grid-cols-5 lg:grid-cols-6',
				className
			)}
			data-testid={testId}
		>
			{Array.isArray(documents) ? (
				...
				<div key={documents.name} className='flex flex-col items-center space-y-4'>
					{documents.type.includes('image') ? (
						<div
							style={{ backgroundImage: `url(${documents.preview})` }}
							className='block w-28 h-28 md:w-24 md:h-24 lg:w-20 lg:h-20 bg-cover bg-center rounded-md'
						/>
					) : (
						<div className='block w-28 h-28 md:w-24 md:h-24 lg:w-20 lg:h-20 flex justify-center items-center'>
